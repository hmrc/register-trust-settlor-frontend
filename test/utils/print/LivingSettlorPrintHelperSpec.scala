      ...
      "include prefix in the message key when defined" when {
        ...
        "UK residency, UK address and Passport pages" in {
          ...
          val expectedAnswerRows = Seq(
            ...
            AnswerRow(label = s"settlorIndividualDateOfBirth.checkYourAnswersLabel", answer = Html("3 February 1996"), changeUrl = Some(SettlorIndividualDateOfBirthController.onPageLoad(index, fakeDraftId).url), labelArg = name.toString),
