  ...
  def utrField(form: UtrFormProvider,
    ...
    "not bind valid UTRs that have been used for other businesses" in {
      ...
      forAll(utrGenerator, intGenerator) {
        (utr, size) =>
          val updatedUserAnswers = (0 until size).foldLeft(emptyUserAnswers)((acc, i) => acc.set(SettlorBusinessUtrPage(i), utr).success.value)
    ...
    "not bind valid UTR if it is the same as the trust UTR" in {
      forAll(utrGenerator) {
        utr =>
          val result = form.apply(prefix, emptyUserAnswers.copy(existingTrustUtr = Some(utr)), 0).bind(Map(fieldName -> utr)).apply(fieldName)
