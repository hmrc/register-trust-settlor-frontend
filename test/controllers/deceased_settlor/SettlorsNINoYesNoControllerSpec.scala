...
class SettlorsNINoYesNoControllerSpec extends SpecBase {
  ...
  lazy val settlorsNINoYesNoRoute = routes.SettlorsNINoYesNoController.onPageLoad(fakeDraftId).url

  val name = FullName("first name", None, "Last name")

  "SettlorsNINoYesNo Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, settlorsNINoYesNoRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[SettlorsNINoYesNoView]
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, settlorsNINoYesNoRoute)
