...
import pages.living_settlor.trust_type.HowDeedOfVariationCreatedPage
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.HowDeedOfVariationCreatedView

class HowDeedOfVariationCreatedControllerSpec extends SpecBase {

  val index = 0

  lazy val deedOfVariationRoute = routes.HowDeedOfVariationCreatedController.onPageLoad(NormalMode, fakeDraftId).url
  ...
  "HowDeedOfVariationCreated Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual
        view(form.fill(DeedOfVariation.values.head), NormalMode, fakeDraftId)(request, messages).toString
