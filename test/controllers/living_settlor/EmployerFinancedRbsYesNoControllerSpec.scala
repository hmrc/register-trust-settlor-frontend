...
import pages.living_settlor.trust_type.EfrbsYesNoPage
import play.api.mvc.Call
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.living_settlor.EmployerFinancedRbsYesNoView

class EmployerFinancedRbsYesNoControllerSpec extends SpecBase {
  ...
  "EmployerFinancedRbsYesNo Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, EmployerFinancedRbsYesNoRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[EmployerFinancedRbsYesNoView]
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, EmployerFinancedRbsYesNoRoute)
