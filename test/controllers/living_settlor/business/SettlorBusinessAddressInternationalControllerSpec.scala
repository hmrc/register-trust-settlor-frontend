...
class SettlorBusinessAddressInternationalControllerSpec extends SpecBase {
  ...
  "SettlorBusinessAddressInternational Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val userAnswers: UserAnswers = emptyUserAnswers.set(SettlorBusinessNamePage(index), name).success.value
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val view = application.injector.instanceOf[SettlorBusinessAddressInternationalView]
    ...
    "redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, settlorBusinessAddressInternationalRoute)
