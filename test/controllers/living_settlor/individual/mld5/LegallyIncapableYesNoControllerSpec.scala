...
class LegallyIncapableYesNoControllerSpec extends SpecBase {
  ...
  private val form: Form[Boolean] = formProvider.withPrefix("settlorIndividualLegallyIncapableYesNo")
  private val index: Int = 0
  private val name: FullName = FullName("First", Some("Middle"), "Last")

  private lazy val onPageLoadRoute: String = routes.LegallyIncapableYesNoController.onPageLoad(NormalMode, index, fakeDraftId).url
  ...
  "CountryOfNationalityYesNo Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseAnswers
        .set(LegallyIncapableYesNoPage(index), validAnswer).success.value
