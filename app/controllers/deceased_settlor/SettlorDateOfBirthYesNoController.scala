          ...
          request.userAnswers.set(SettlorDateOfBirthYesNoPage, value) match {
            case Success(updatedAnswers) =>
              registrationsRepository.set(updatedAnswers).map { _ =>
                Redirect(navigator.nextPage(SettlorDateOfBirthYesNoPage, draftId)(updatedAnswers))
              }
            case Failure(_) => {
              logger.error("[SettlorDateOfBirthYesNoController][onSubmit] Error while storing user answers")
