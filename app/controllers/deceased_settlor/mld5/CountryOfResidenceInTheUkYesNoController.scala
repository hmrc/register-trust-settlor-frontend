    ...
    (actions.authWithData(draftId) andThen requireName(draftId)).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            request.userAnswers.set(CountryOfResidenceInTheUkYesNoPage, value) match {
              case Success(updatedAnswers) =>
                registrationsRepository.set(updatedAnswers).map { _ =>
                  Redirect(navigator.nextPage(CountryOfResidenceInTheUkYesNoPage, draftId)(updatedAnswers))
                }
              case Failure(_)              =>
