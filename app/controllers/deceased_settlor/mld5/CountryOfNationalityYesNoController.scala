...
class CountryOfNationalityYesNoController @Inject() (
  ...
  private val form: Form[Boolean] = yesNoFormProvider.withPrefix("5mld.countryOfNationalityYesNo")
    ...
    (actions.authWithData(draftId) andThen requireName(draftId)).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            request.userAnswers.set(CountryOfNationalityYesNoPage, value) match {
              case Success(updatedAnswers) =>
                registrationsRepository.set(updatedAnswers).map { _ =>
                  Redirect(navigator.nextPage(CountryOfNationalityYesNoPage, draftId)(updatedAnswers))
                }
              case Failure(_)              =>
