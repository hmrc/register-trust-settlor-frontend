...
class SettlorDateOfDeathYesNoController @Inject()(
  ...
  def onPageLoad(draftId: String): Action[AnyContent] = (actions.authWithData(draftId) andThen requireName(draftId)) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(SettlorDateOfDeathYesNoPage) match {
        case None => form
  ...
  def onSubmit(draftId: String): Action[AnyContent] = (actions.authWithData(draftId) andThen requireName(draftId)).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          request.userAnswers.set(SettlorDateOfDeathYesNoPage, value) match {
            case Success(updatedAnswers) =>
              registrationsRepository.set(updatedAnswers).map { _ =>
                Redirect(navigator.nextPage(SettlorDateOfDeathYesNoPage, draftId)(updatedAnswers))
              }
            case Failure(_) => {
              logger.error("[SettlorDateOfDeathYesNoController][onSubmit] Error while storing user answers")
