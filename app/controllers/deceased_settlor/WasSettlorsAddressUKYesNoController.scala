    ...
    (actions.authWithData(draftId) andThen requireName(draftId)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            request.userAnswers.set(WasSettlorsAddressUKYesNoPage, value) match {
              case Success(updatedAnswers) =>
                registrationsRepository.set(updatedAnswers).map { _ =>
                  Redirect(navigator.nextPage(WasSettlorsAddressUKYesNoPage, draftId)(updatedAnswers))
                }
              case Failure(_)              =>
                logger.error("[WasSettlorsAddressUKYesNoController][onSubmit] Error while storing user answers")
