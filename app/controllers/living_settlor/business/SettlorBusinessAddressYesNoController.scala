...
class SettlorBusinessAddressYesNoController @Inject()(
  ...
  val form: Form[Boolean] = yesNoFormProvider.withPrefix("settlorBusinessAddressYesNo")
  ...
  def onSubmit(mode: Mode, index: Int, draftId: String): Action[AnyContent] = (actions.authWithData(draftId) andThen requireName(index, draftId)).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(SettlorBusinessAddressYesNoPage(index), value))
            _              <- registrationsRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(SettlorBusinessAddressYesNoPage(index), mode, draftId)(updatedAnswers))
        }
      )
  }
}
