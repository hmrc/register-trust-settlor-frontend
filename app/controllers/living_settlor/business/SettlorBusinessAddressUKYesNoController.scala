...
import pages.living_settlor.business.SettlorBusinessAddressUKYesNoPage
    ...
    (actions.authWithData(draftId) andThen requireName(index, draftId)).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            request.userAnswers.set(SettlorBusinessAddressUKYesNoPage(index), value) match {
              case Success(updatedAnswers) =>
                registrationsRepository.set(updatedAnswers).map { _ =>
                  Redirect(navigator.nextPage(SettlorBusinessAddressUKYesNoPage(index), draftId)(updatedAnswers))
                }
              case Failure(_)              =>
