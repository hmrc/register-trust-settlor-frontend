...
import pages.living_settlor.individual.{SettlorIndividualDateOfBirthYesNoPage, SettlorIndividualNamePage}
...
import views.html.living_settlor.individual.SettlorIndividualDateOfBirthYesNoView
...
class SettlorIndividualDateOfBirthYesNoController @Inject() (
  ...
  private val form: Form[Boolean] = yesNoFormProvider.withPrefix("settlorIndividualDateOfBirthYesNo")
    ...
    (actions.authWithData(draftId) andThen requireName(index, draftId)).async { implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            request.userAnswers.set(SettlorIndividualDateOfBirthYesNoPage(index), value) match {
              case Success(updatedAnswers) =>
                registrationsRepository.set(updatedAnswers).map { _ =>
                  Redirect(navigator.nextPage(SettlorIndividualDateOfBirthYesNoPage(index), draftId)(updatedAnswers))
                }
              case Failure(_)              =>
                logger.error("[SettlorIndividualDateOfBirthYesNoController][onSubmit] Error while storing user answers")
