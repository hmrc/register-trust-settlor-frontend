...
class LivingSettlorPrintHelper @Inject()(answerRowConverter: AnswerRowConverter,
  ...
  override def answerRows(index: Int, draftId: String, prefix: Option[String] = None)
    ...
    Seq(
      ...
      bound.dateQuestion(SettlorIndividualDateOfBirthPage(index), s"settlorIndividualDateOfBirth", SettlorIndividualDateOfBirthController.onPageLoad(index, draftId).url),
