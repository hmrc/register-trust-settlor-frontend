...
class DeceasedSettlorNavigator @Inject() (config: FrontendAppConfig) extends Navigator {
  ...
  def simpleNavigation(draftId: String): PartialFunction[Page, UserAnswers => Call] = {
    case SettlorsNamePage                 => _ => rts.SettlorDateOfDeathYesNoController.onPageLoad(draftId)
    case SettlorDateOfDeathPage           => _ => rts.SettlorDateOfBirthYesNoController.onPageLoad(draftId)
    case CountryOfResidencePage           => ua => navigateAwayFromCountryOfResidencyQuestions(draftId)(ua)
    case SettlorsUKAddressPage            => _ => rts.DeceasedSettlorAnswerController.onPageLoad(draftId)
    case SettlorsInternationalAddressPage => _ => rts.DeceasedSettlorAnswerController.onPageLoad(draftId)
    case CountryOfNationalityPage         => ua => navigateAwayFromNationalityQuestions(ua.isTaxable, draftId)
    case DeceasedSettlorAnswerPage        => _ => Call(GET, config.registrationProgressUrl(draftId))
  }

  def yesNoNavigation(draftId: String): PartialFunction[Page, UserAnswers => Call] = {
    case SettlorDateOfDeathYesNoPage          =>
      yesNoNav(
    ...
    case SettlorsLastKnownAddressYesNoPage    =>
      yesNoNav(
    ...
    case WasSettlorsAddressUKYesNoPage        =>
      yesNoNav(
    ...
    case CountryOfNationalityInTheUkYesNoPage =>
      answers =>
        yesNoNav(
  ...
  private def mldDependentSimpleNavigation(draftId: String): PartialFunction[Page, UserAnswers => Call] = {
    case SettlorsDateOfBirthPage            => ua => mld5Rts.CountryOfNationalityYesNoController.onPageLoad(draftId)
    case SettlorNationalInsuranceNumberPage => _ => mld5Rts.CountryOfResidenceYesNoController.onPageLoad(draftId)
  }

  private def mldDependentYesNoNavigation(draftId: String): PartialFunction[Page, UserAnswers => Call] = {
    case SettlorDateOfBirthYesNoPage        =>
      ua =>
        yesNoNav(
    ...
    case SettlorsNationalInsuranceYesNoPage =>
      ua =>
        yesNoNav(
