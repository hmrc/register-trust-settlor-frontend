...
object SettlorNavigator {
  ...
  def addSettlorRoute(answers: UserAnswers, draftId: String): Call = {
    ...
    routes.filter(_._1.size < MAX) match {
      ...
      case _             =>
        routeToIndex(
          answers,
          controllers.living_settlor.routes.SettlorIndividualOrBusinessController.onPageLoad,
          draftId
        )
    }
  }

  def addSettlorNowRoute(
  ...
  private def routeToIndividualSettlorIndex(answers: UserAnswers, draftId: String, index: Option[Int]): Call =
    routeToIndex(
      answers,
      controllers.living_settlor.individual.routes.SettlorAliveYesNoController.onPageLoad,
      draftId,
      index
    )
  ...
  private def routeToSettlorProtectorIndex(answers: UserAnswers, draftId: String, index: Option[Int]): Call =
    routeToIndex(
      answers,
      controllers.living_settlor.business.routes.SettlorBusinessNameController.onPageLoad,
      draftId,
      index
    )

  private def routeToIndex[T <: SettlorViewModel](
    answers: UserAnswers,
